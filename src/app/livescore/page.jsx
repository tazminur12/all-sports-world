...
import { RefreshCw, Clock, AlertCircle, Wifi, ChevronDown } from 'lucide-react';
...
function SportSelector({ activeSport, onChange }) {
  ...
  const currentSport = ALL_SPORTS.find((s) => s.label === activeSport);
        ...
        {open && (
          ...
          >
            ...
            {SPORT_CATEGORIES.slice(1).map((cat) => {
              const sportsInCat = ALL_SPORTS.filter((s) => s.category === cat);
...
export default function LiveScorePage() {
  ...
  const {
    ...
    lastUpdate, isUpdating, usingMock,
  ...
  const activeSports = useMemo(() => {
    ...
    return ALL_SPORTS.filter((s) => sportSet.has(s.label));
  }, [scores]);
    ...
    return (
      <div className="min-h-screen pt-28 flex flex-col items-center
        ...
        <p className="text-textSecondary text-sm animate-pulse">
          Fetching live scores from 20+ sports...
        </p>
      </div>
    );
  }
        ...
        <div className="flex items-center justify-center gap-2 mt-3">
          {usingMock ? (
            ...
            <span className="flex items-center gap-1.5 text-xs text-accentGreen">
              ...
              Connected · {activeSports.length} sports live
            </span>
          )}
        </div>
      </motion.div>
